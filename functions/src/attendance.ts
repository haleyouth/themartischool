...
export const onAttendanceWrite = onDocumentWritten(
  { document: 'attendance/{sessionId}', region: 'us-central1' },
  async (event) => {
    ...
    const beforeRecords = (before?.records ?? {}) as Record<string, Record_>
    const afterRecords = (after.records ?? {}) as Record<string, Record_>
    ...
    for (const studentId of removed) {
      batch.delete(
        db.doc(`attendanceEntries/${studentId}_${after.classId}_${after.sessionDate}`),
      )
    }
    ...
    await Promise.all(
      changed.map(async (studentId) => {
        const studentSnap = await db.doc(`students/${studentId}`).get()
        if (!studentSnap.exists) return
        const uid = studentSnap.data()!.uid as string
        await db
    ...
    await Promise.all(
      affected.map(async (studentId) => {
        ...
        rows.docs.forEach((doc) => {
          const status = doc.data().status as AttendanceStatus
