...
const fieldBase =
  'w-full rounded-xl border bg-white px-3.5 text-sm text-ink placeholder:text-ink-400 transition-all duration-200 focus:outline-none focus:ring-4 disabled:cursor-not-allowed disabled:bg-cream-200 disabled:text-ink-400'

const fieldOk = 'border-ink-200 focus:border-marti-500 focus:ring-marti-500/10'
const fieldError = 'border-magenta-400 focus:border-magenta-500 focus:ring-magenta-500/10'
...
function FieldShell({
  ...
  return (
    <div className={cn('space-y-1.5', className)}>
      ...
      {children}
      {error ? (
        <p className="flex items-center gap-1.5 text-xs font-medium text-magenta-600">
...
export function Checkbox({
  ...
  return (
    <div className="space-y-1.5">
      ...
      {error && (
        <p className="flex items-center gap-1.5 pl-7 text-xs font-medium text-magenta-600">
          <AlertCircle className="h-3.5 w-3.5 shrink-0" aria-hidden />
          {error}
        </p>
      )}
