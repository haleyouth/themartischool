      ...
      >
        ...
        {unread.length > 0 && (
          <motion.span
            initial={{ scale: 0 }}
            animate={{ scale: 1 }}
            className="absolute -right-0.5 -top-0.5 flex h-4 min-w-4 items-center justify-center rounded-full bg-magenta-600 px-1 text-[10px] font-bold text-white"
          >
      ...
      <AnimatePresence>
        {open && (
          <motion.div
            ...
            className="absolute right-0 top-full z-50 mt-2 w-[min(22rem,calc(100vw-2rem))] overflow-hidden rounded-2xl border-2 border-ink bg-white shadow-card"
          >
            <div className="flex items-center justify-between border-b border-ink-200 px-4 py-3">
              <p className="text-sm font-bold text-ink">{t('dash.notifications')}</p>
              {unread.length > 0 && (
                <button
                  type="button"
                  onClick={markAllRead}
                  ...
                  {notifications.map((item) => (
                    <li key={item.id}>
                      <button
                        ...
                        className={cn(
                          'flex w-full gap-3 px-4 py-3 text-left transition-colors hover:bg-cream-200',
            ...
            <button
              ...
              className="block w-full border-t border-ink-200 bg-cream-200/60 px-4 py-3 text-center text-xs font-semibold text-marti-600 transition-colors hover:bg-ink-100"
            >
              {t('common.viewAll')}
            </button>
          </motion.div>
